# Determine Tunnel Maximum MTU and Configuring OpenVPN Server
...
## You must have a working VPN connection that you can ping the client from the OpenVPN server

You can find the client's IP address in the OpenVPN server log. The client name is normallay what you named your opvn client config file (for example: client-thinkpad.ovpn).  The client I'm interesed testing in is `client-thinkpad`,  Do a search for the clients IP address, I use the journal command to search, it will be the "pool return IPv4=xxx.xxx.xxx.xxx".  
